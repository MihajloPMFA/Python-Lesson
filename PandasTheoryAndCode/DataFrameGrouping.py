...
#                If you tried to print this out on your own you wouldnt exactly get a DataFrame but a adress to one.
...
#       2) The next function .size() is actually a good example we can use. If you were to print out this line of code:
            filtered_data.groupby(['platform', 'genre']).size() # you would be shown a grouped DataFrame, where the size paramater would be telling you how many instances of each 'genre' were for every 'platform'
...
#       4).fillna(0) this is where this function comes in. It will go throught each value inside the DataFrame, and when it encounters the 'NaN' message, it will replace it with 0. 
...
# If you are wondering why we will be sorting by the 'Action' column, open the 'example.png' file again an notice that, from the left to right, the number of games that fall in the 'Action' genre is decreasing.
