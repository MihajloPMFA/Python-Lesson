...
#       2) Using the list, filter out the unneccesary consoles using the next command:
...
# Because we want to filter our DataFrame by the platforms that are in the list we created before, we can use the .isin(platforms)
...
#because this will also return a DataFrame we need to ige it a variable to return the value to

filtered_data = data[data['platform'].isin(platforms)]

# will give the following result - see file "filtered_data2.py"
